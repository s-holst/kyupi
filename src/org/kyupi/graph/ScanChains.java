	...
	public ScanChains(Graph scanned_netlist) {
		...
		for (Node n : all_cells.keySet()) {
			...
			if (prev.isInput()) {
				...
				if (prev_cell == null) {
					log.error("Encountered non-scan-cell on scan path: " + prev.queryName());
				} else {
					if (cell.prev != null) {
		...
		log.info("Number of scan chains: " + chains.length);
		...
		for (ScanCell head : chain_heads) {
			...
			log.info("Length of chain " + chainNr + ": " + length);
		...
		for (Node n : all_cells.keySet()) {
			log.warn("Unconnected scan-cell: " + n.queryName());
		}
	}

	private Node backtraceToIntf(Node node) {
		if (node.level() > 0) {
			if (!node.isType(Library.TYPE_BUF))
				log.warn("Encountered non-buffer gate on scan path: " + node.queryName() + " " + Long.toHexString(node.type()));
